...
A multi-component signal can be decomposed by LMD into a succession of PFs and a residue. Each PF is a mono component that is the result of an envelope signal and a frequency modulated signal. A two-tier method is utilized to perform the decomposition based on the PF specification.First, the inner cycle is used to conduct a thorough computation of the PF. Second, in the outer cycle, the signal is decomposed depending on the iterations. In the following sections the methods have been broken down into steps that LMD technique requires to complete the analysis on a given signal x(t). Consider the following synthetic signal $x (t)$ of the form :
...
The local mean function $m_{11}(t)$ can be formed by using moving averaging to smooth the local means $m_i$.
...
A corresponding envelope estimate $a_i$ is given by:
...
Similarly, the envelope estimate $a_i$ is smoothed and corresponding envelope function $a_{11}$(t) is formed.
...
The local mean function $m_{11}$(t) is subtracted from the original signal $x(t)$ and the estimated zero-mean signal $h_{11}$(t) is given by
...
Then, a frequency modulated signal $s_{11}$(t) can be obtained as:
...
Repeat Steps 1 -- 3 to get the envelope estimate $a_{12}$(t) of $s_{11}$(t) . If the envelope function $a_{12}$(t) =1, stop the procedure and take $s_{12}$(t) as the first purely frequency modulated (FM) signal. Otherwise, let $s_{11}$(t) be the original signal and repeat Steps 1 -- 4 n times until the envelope function $a_{1(n+1)}$(t) of $s_{1(n)}$(t) satisfies $a_{1(n+1)}$(t)=1
...
Compute the residue signal $u_1(t)$ = $x(t)$-$PF_1(t)$ . Take $u_1(t)$ as a new signal and repeat the above procedure n times until $u_n(t)$ does not contain oscillation.

This is the called outer loop of LMD. Finally, the signal x(n) can be reconstructed according to the equation:
...
In this section we will take a simulated multi-component AM--FM signal x(t) and apply the LMD method on it. The time duration of the signal is 600s with sampling frequency 1 Hz.
...
-   `pricolor_plot`: Line colour for plots
...
In the plot above two PFs and a residue signal have been generated and represented using aplot. PF1(t) is the component corresponding to x1(t) and PF2 is the component corresponding to x2(t).PF1(t) and PF2(t) match well with x1(t) and x2(t), respectively. However, the residue signal contains some unexpected peaks, which demonstrates that the demodulation is not perfect.


# Drawbacks of LMD

LMD shows good ability in demodulating amplitude and frequency modulated signals. However, it is not perfect. It has shortcomings, e.g., end effects and mode mixing. For better understanding, the decomposition results in Fig. 9 are utilized to illustrate the end effect and mode mixing problems. In Fig. 10 we can observe distortions in the two ends of PF1(t). This phenomenon is called the end effect of LMD. In addition, we can see another distortion at around 500 s. This is called mode mixing of LMD. ***To mitigate these issues, several variations of LMD (like RLMD,IELMD,OLMD etc) have been proposed by researchers, however, these methods have not been included in this package.***
...
* Decomposition of ECG Signal

# References

* D. Kim and H.-S. Oh, “EMD: A package for empirical mode decomposition and Hilbert spectrum,” The R Journal, 01-Jun-2009. [Online]. Available: https://journal.r-project.org/articles/RJ-2009-002/. [Accessed: 17-Sep-2022]. 
J. Cheng, Y. Yang, and Y. Yang, “A rotating machinery fault diagnosis method based on local mean decomposition,” Digital Signal Processing, 19-Oct-2011. [Online]. Available: https://www.sciencedirect.com/science/article/pii/S1051200411001448. [Accessed: 17-Sep-2022]. 
